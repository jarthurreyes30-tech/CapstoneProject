...
import { ArrowLeft, Edit, Share2, MoreVertical, MapPin, TrendingUp, Heart, Calendar, Users } from "lucide-react";
...
export default function Profile() {
  ...
  const fetchStats = async () => {
    try {
      ...
      const followingResponse = await api.get("/me/following");
      
      setStats({
        totalDonated: 0, // TODO: Fetch from donations API
        campaignsSupported: 0, // TODO: Fetch from campaigns API
        recentDonations: 0, // TODO: Fetch from recent donations
        followedCharities: followingResponse.data.length,
      });
    } catch (error) {
  ...
  const statItems = [
    ...
    },
    {
      ...
      value: stats.followedCharities.toString(),
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-4 mb-8">
          {statItems.map((item, index) => (
            <Card
              ...
              className={`relative overflow-hidden bg-background/40 border-border/40 hover:shadow-lg hover:-translate-y-1 transition-all duration-150 ${item.onClick ? 'cursor-pointer' : ''} rounded-2xl ring-1 ${item.ring} hover:ring-2 active:scale-[0.98]`}
            >
