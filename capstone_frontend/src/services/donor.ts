...
class DonorService {
  ...
  async getDashboardStats(): Promise<DonorStats> {
    try {
      ...
      console.warn('Dashboard endpoint failed, falling back to calculating from donations:', error);
      
      // Fallback: Fetch donations and calculate stats from them
  ...
  async getSuggestedCampaigns(limit: number = 3): Promise<SuggestedCampaign[]> {
    try {
      ...
      // Fetch campaigns from charities until we fill the desired limit
      const allCampaigns: SuggestedCampaign[] = [];

      // Reasonable upper bound to avoid too many requests
      const MAX_CHARITIES = Math.min(charities.length, Math.max(limit * 5, 10));

      for (const charity of charities.slice(0, MAX_CHARITIES)) {
        try {
          ...
          if (campaignsRes.status === 200) {
            ...
            if (campaigns && Array.isArray(campaigns)) {
              ...
              // Stop early if we have enough to display
